...
#include "optimization/ceres_pose.h"
#include "optimization/cost_functions/acceleration_cost_function_3d.h"
#include "optimization/cost_functions/rotation_cost_function_3d.h"
...
ImuBasedPoseExtrapolator::InitializeWithImu(
  ...
  extrapolator->AddPlan(common::FromUniversal(0),nullptr);
  return extrapolator;
}
...
void ImuBasedPoseExtrapolator::AddPose(const common::Time time,
  ...
  while (timed_pose_queue_.size() > 3 &&
    ...
    // LOG(INFO)<<timed_pose_queue_.size();
    // plane_.pop_front();
...
void ImuBasedPoseExtrapolator::AddImuData(const sensor::ImuData& imu_data) {
  ...
  // CHECK(common::ToSeconds(imu_data.time -imu_data_.back().time) <
  //       0.005);
  imu_data_.push_back(imu_data);
  TrimImuData();
}

void ImuBasedPoseExtrapolator::AddPlan(common::Time time,
                                       std::shared_ptr<Eigen::Vector4f> plan) {
  plane_.push_back(plan);
}
...
ImuBasedPoseExtrapolator::ExtrapolatePosesWithGravity(
  ...
  for (size_t i = 0; i < timed_pose_queue_.size(); i++) {
    ...
    // LOG(INFO) << plane_.size() << " " << timed_pose_queue_.size();
    // LOG(INFO) << nodes.at(i).translation()[2] << " "
    //           << nodes.at(i).translation()[1] << " "
    //           << nodes.at(i).translation()[0];
    // if (plane_[i]) {
    //   plane_cost_fuction_.AddToProblem(&problem, &nodes.at(i), *plane_[i]);
    // }
  }
  // LOG(INFO)<<nodes.back().translation()[2];
  ...
  for (size_t i = 1; i < nodes.size(); i++) {
    ...
    if ((i + 1) < nodes.size()) {
      ...
      // LOG(INFO)<<common::ToSeconds(first_duration);
      // LOG(INFO)<<delta_velocity;
      // LOG(INFO)<<delta_velocity/common::ToSeconds(first_duration);
      problem.AddResidualBlock(
      ...
      // LOG(INFO) << result.delta_velocity ;
      // LOG(INFO) << (Eigen::Quaterniond(nodes.at(i - 1).rotation()) *
      //               result.delta_velocity) ;
      // LOG(INFO) << (Eigen::Quaterniond(nodes.at(i - 1).rotation()) *
      //               result.delta_velocity) +
      //                  9.8 * Eigen::Vector3d::UnitZ() *
      //                      common::ToSeconds(first_duration);
      // int x = 0;
      // std::cin>>x;
      if (options_.imu_base_pre()) {
        problem.AddResidualBlock(
            AccelerationPreCostFunction3D::CreateAutoDiffCostFunction(
                options_.imu_acceleration_weight(), result.delta_translation,
                result.delta_velocity, common::ToSeconds(first_duration)),
            nullptr, nodes.at(i - 1).rotation(), nodes.at(i - 1).translation(),
            nodes.at(i - 1).velocity(), nodes.at(i).velocity(),
            nodes.at(i).translation(), &gravity_constant,
            imu_calibration.data());
      }

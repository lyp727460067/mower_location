...
//   common::Time GetLastExtrapolatedTime() const override;
  ...
  void AddPlan(common::Time time, std::shared_ptr<Eigen::Vector4f> plan);
 ...
 private:
  ...
  std::vector<transform::Rigid3f> InterpolatePoses(
      const transform::TimestampedTransform& start,
      const transform::TimestampedTransform& end,
      const std::vector<common::Time>::const_iterator times_begin,
      const std::vector<common::Time>::const_iterator times_end);

  std::deque<transform::TimestampedTransform> timed_pose_queue_;
  std::deque<transform::TimestampedTransform>
      previous_solution_;

  std::deque<sensor::ImuData> imu_data_;
  std::deque<sensor::OdometryData> odometry_data_;
  std::deque<std::shared_ptr<Eigen::Vector4f> > plane_;

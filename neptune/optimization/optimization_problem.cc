...
#include "neptune/mapping/internal/3d/imu_integration.h"
...
void OptimizationProblem3D::Solve(
  ...
  if (options_.use_floor_costraint()) {
    for (auto node_it = std::next(node_data_.begin());
         node_it != node_data_.end(); ++node_it) {
      mapping::PlaneCostFunction plan_cost_function;
      if (node_it->data.floor != nullptr) {
        std::shared_ptr<FloorData> floor = std::make_shared<FloorData>(
            FloorData{sensor::PointCloud(), *node_it->data.floor});
        // LOG(INFO) << "add floor param******************8";
        plan_cost_function.AddToProblem(
            &problem, &C_nodes.at(node_it->id), floor,
            transform::Rigid3d::Identity(),
            std::array<double, 3>{{options_.floor_constraint_weight_0(),
                                   options_.floor_constraint_weight_1(),
                                   options_.floor_constraint_weight_2()}});
      }
    }
  }
  // Solve.

#ifndef NEPTUTN_TRANSFORM_RIGID_TRANSFORM_H_
#define NEPTUTN_TRANSFORM_RIGID_TRANSFORM_H_
...
template <typename T>
std::ostream& operator<<(std::ostream& os, const transform::Rigid2<T>& rigid) {
...
// This is needed for gmock.
template <typename T>
std::ostream& operator<<(std::ostream& os, const transform::Rigid3<T>& rigid) {
  os << rigid.DebugString();
  return os;
}
